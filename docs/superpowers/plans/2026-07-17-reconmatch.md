...
- [x] Step 1: `uv init --lib`-style pyproject copied from docval's shape (name reconmatch, deps: pydantic, gradio; dev: pytest, ruff). `uv sync --dev`.
- [x] Step 2: Failing test: construct each model; CSV round-trip loaders reject float-looking parse only via Decimal("…"); malformed row raises `RowError` carrying row number.
```python
...
```
- [x] Step 3: Implement schema + loaders (csv module, `Decimal(raw)`, `date.fromisoformat`). Run tests → pass. `ruff check`.
- [x] Step 4: Commit `feat: schema and CSV loaders`.
...
```
- [x] Step 2: Implement; run; commit `feat: synthetic ledger/statement generator with truth`.
...
- [x] Step 1: Failing tests: exact pair matches at conf ≥0.95; two identical-amount candidates resolve deterministically under input shuffle (property: `match(shuffled) == match(original)`); no line reused across pairs.
```python
...
```
- [x] Step 2: Implement; commit `feat: tier-1 exact matcher with deterministic greedy assignment`.
...
- [x] Step 1: Failing tests: 2-day-lagged line matches at tier 2 with confidence in (0.6, 0.95); a same-amount line beyond the window does NOT match; tier 1 wins over tier 2 for the same entry.
- [x] Step 2: Implement; commit `feat: tier-2 date-windowed matching`.
...
- [x] Step 1: Failing tests: a 3-way split sums and matches with `line_ids` sorted; k>max_split does not match; split candidates lose to a tier-1 candidate competing for the same lines.
- [x] Step 2: Implement; add the whole-engine property test on generated data:
```python
...
```
- [x] Step 3: Commit `feat: tier-3 split-payment matching`.
...
- [x] Step 1: Failing tests: one handcrafted fixture per category asserting category, related_id, and that suggestions mention the amounts involved.
- [x] Step 2: Implement; commit `feat: break classification and resolution suggestions`.
...
- [x] Step 1: Failing tests with tiny handcrafted pred/truth: perfect prediction ⇒ f1 1.0 and rate 1.0; one false match at high confidence caps auto_match_rate below 1.0; threshold returned actually achieves the floor.
- [x] Step 2: Implement; run `uv run python -m eval.run_eval --seeds 100-149 --out eval/results` once; check in the aggregates as `eval/baseline_ci.json` (offline, deterministic, so CI can regenerate and compare cheaply).
- [x] Step 3: CI workflow `.github/workflows/ci.yml`: copy docval's shape minus apt deps — pytest, ruff, then regenerate held-out eval and `eval/compare.py` against baseline.
- [x] Step 4: Commit `feat: eval harness with auto-match-rate@0.99-precision + CI gate`.
...
- [x] Step 1: Failing test on the fixture: row count matches, a debit row comes out negative, Decimal type preserved.
- [x] Step 2: Implement; commit `feat: DocVal extraction adapter`.
...
- [x] Step 1: Failing tests: `run_reconcile(ledger_path, statement_path)` returns non-empty tables on samples; malformed CSV path returns "❌" message not exception.
- [x] Step 2: Implement; verify by launching locally and driving with `gradio_client` (samples + upload flow), same as docval verification.
- [x] Step 3: Commit `feat: Gradio reconciliation demo`.
...
- [x] Step 1: Write files; `bash -n`-equivalent sanity (docker build unavailable locally — mirror the verified docval pattern).
- [x] Step 2: Full `uv run pytest`, `ruff check`, commit `docs: README with held-out numbers + deploy scaffolding`.
